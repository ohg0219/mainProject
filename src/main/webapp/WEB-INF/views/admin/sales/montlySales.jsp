...
<title>thisisthat - 월별 매출현황</title>
...
$(document).ready(function () {
	$("#excelDownloadBtn").on("click",function(e){
		...
		if(sDate==""||eDate==""){
			...
			location.href="/admin/excelDownload.mdo?sDate="+sDate+"&eDate="+eDate+"&type=monthly";
		}
	});
	$("#pdfDownloadBtn").on("click",function(e){
		...
		if(sDate==""||eDate==""){
			...
			location.href="/admin/pdfDownload.mdo?sDate="+sDate+"&eDate="+eDate+"&type=monthly";
		}
	});
});
function getData(){
	...
	location.href="/admin/montlySales.mdo?sDate="+sDate+"&eDate="+eDate;
}
</script>
</head>
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <h1 class="h3 mb-2 text-gray-800">
                   	...
                   	<div>
                   		기간 선택 : <input class="btn btn-dark" type="text" id="sDate" autocomplete="off" name="sDate" value="${startMonth }"> ~ <input class="btn btn-dark" type="text" id="eDate" autocomplete="off" name="eDate" value="${endMonth }">
                         ...
                         <c:choose>
                         	<c:when test="${not empty monthly }">
								<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
									<h6 class="m-0 font-weight-bold text-primary">${startMonth } ~ ${endMonth } 조회 결과</h6>
								</div>
								<!-- Card Body -->
								<div class="card-body">
...
//Set new default font family and font color to mimic Bootstrap's default styling
 ...
 var myLineChart = new Chart(ctx, {
   type: 'line',
   data: {
     labels: [<c:forEach items="${monthly}" var="month">"${month.groupString}",</c:forEach>],
     datasets: [{
       label: "",
       lineTension: 0.3,
       ...
       pointHitRadius: 10,
       pointBorderWidth: 2,
       data: [<c:forEach items="${monthly}" var="sales">"${sales.subtotal}",</c:forEach>],
     }],
   },
   options: {
