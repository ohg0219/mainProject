...
<title>thisisthat - 일별 매출현황</title>
...
$(document).ready(function () {
	$("#excelDownloadBtn").on("click",function(e){
		...
		if(sDate==""||eDate==""){
			...
			location.href="/admin/excelDownload.mdo?sDate="+sDate+"&eDate="+eDate+"&type=daily";
		}
	});
	$("#pdfDownloadBtn").on("click",function(e){
		...
		if(sDate==""||eDate==""){
			...
			location.href="/admin/pdfDownload.mdo?sDate="+sDate+"&eDate="+eDate+"&type=daily";
		}
	});
});
function getData(){
	...
	location.href="/admin/dailySales.mdo?sDate="+sDate+"&eDate="+eDate;
}
...
//Set new default font family and font color to mimic Bootstrap's default styling
 ...
 var myLineChart = new Chart(ctx, {
   type: 'line',
   data: {
     labels: [<c:forEach items="${daily}" var="day">"${day.groupString}",</c:forEach>],
     datasets: [{
       label: "",
       lineTension: 0.3,
       ...
       pointHitRadius: 10,
       pointBorderWidth: 2,
       data: [<c:forEach items="${daily}" var="sales">"${sales.subtotal}",</c:forEach>],
     }],
   },
   options: {
